...
(function () {
  ...
  // ---- Lenis smooth scroll, wired to ScrollTrigger's updater (the pattern
  // documented at lenis.dev) — skipped entirely under reduced-motion (native
  // scroll instead) and if the CDN failed to load (typeof check), so this
  // never blocks scrolling if anything goes wrong. ----
  if (!reducedMotion && typeof window.Lenis !== 'undefined') {
    ...
    } catch (err) { /* smooth scroll is an enhancement, never a requirement — native scroll still works */ }
  }
  ...
  var EXPERIMENTAL_TILT_PRESETS = {
    'glass-3d-space': 1, 'layered-depth': 1, 'geometric-playground': 1, 'webgl-abstract': 1,
    'flowing-strands': 1, 'botanical-petals': 1, 'sculptural-forms': 1, 'structural-grid': 1, 'atmospheric-depth': 1, 'floating-objects': 1,
  };
  ...
  if (!reducedMotion) {
    ...
    if (experimentalLayer) {
      ...
      // ---- Real Three.js/WebGL scenes — a small library of category-driven
      // "recipes" (see experimentalMotion.js's scene3d field), each a
      // distinct abstract geometry family matching a business category's own
      // visual identity (never a literal photo/product/premises). Every
      // recipe shares the same camera/lighting/pointer/scroll/resize/pause
      // scaffold in start3dScene below — a recipe only builds its objects and
      // returns a per-frame animate() callback, so the safety properties
      // (capped counts, off-screen pause, resize handling, WebGL-failure
      // fallback) are guaranteed for every recipe by construction, not
      // something to re-derive per scene. Lazy-loaded — the library is only
      // fetched when a preset with the particle-field technique is actually
      // present on the page. ----
      ...
      // Each function: (THREE, scene, accentColor, isMobile) -> populates
      // `scene` and returns { animate(scrollProgress), dollyRange }.
      var SCENE_RECIPES = {
        // Consulting/tech — depth-separated network nodes with connecting
        // lines that appear/disappear as nodes drift into range, plus
        // floating translucent glass planes at distinct depths.
        network: function (THREE, scene, accentColor, isMobile) {
          ...
          var planes = [-6, -1.5, 3.5].map(function (z, idx) {
            var mat = new THREE.MeshPhysicalMaterial({ color: 0xffffff, transparent: true, opacity: 0.07, roughness: 0.25, metalness: 0.05, side: THREE.DoubleSide });
        ...
        // Salon/beauty — silky, reflective ribbon-like strands, each a fixed
        // wavy tube (cheap: rigid mesh, no per-frame re-curving) that bobs
        // and sways gently at its own phase/speed.
        strands: function (THREE, scene, accentColor, isMobile) {
          var STRAND_COUNT = isMobile ? 3 : 7;
          var strands = [];
          for (var i = 0; i < STRAND_COUNT; i++) {
            var points = [];
            var segs = 40, amp = 1.2 + Math.random() * 0.8, freq = 1.5 + Math.random(), phase = Math.random() * Math.PI * 2;
            for (var s = 0; s <= segs; s++) {
              var t = s / segs;
              points.push(new THREE.Vector3((t - 0.5) * 14, Math.sin(t * Math.PI * freq + phase) * amp, Math.cos(t * Math.PI * freq * 0.7 + phase) * amp * 0.8));
            }
            var curve = new THREE.CatmullRomCurve3(points);
            var tubeGeo = new THREE.TubeGeometry(curve, 64, 0.045, 8, false);
            var mat = new THREE.MeshPhysicalMaterial({ color: accentColor, metalness: 0.6, roughness: 0.15, transparent: true, opacity: 0.85, side: THREE.DoubleSide });
            var mesh = new THREE.Mesh(tubeGeo, mat);
            mesh.position.set(0, (Math.random() - 0.5) * 4, (i - (STRAND_COUNT - 1) / 2) * 1.1);
            mesh.userData = { baseY: mesh.position.y, bobSpeed: 0.3 + Math.random() * 0.3, bobPhase: Math.random() * Math.PI * 2 };
            scene.add(mesh);
            strands.push(mesh);
          }
          var clock = 0;
          return {
            ...
            animate: function () {
              clock += 0.01;
              strands.forEach(function (mesh) {
                mesh.position.y = mesh.userData.baseY + Math.sin(clock * mesh.userData.bobSpeed + mesh.userData.bobPhase) * 0.35;
                mesh.rotation.z = Math.sin(clock * 0.2 + mesh.userData.bobPhase) * 0.08;
              });
            },
          };
        },

        // Florist — abstract petal-like oval forms tumbling and drifting at
        // different depths.
        petals: function (THREE, scene, accentColor, isMobile) {
          var PETAL_COUNT = isMobile ? 8 : 20;
          var petals = [];
          var geo = new THREE.CircleGeometry(0.9, 10);
          for (var i = 0; i < PETAL_COUNT; i++) {
            var mat = new THREE.MeshPhysicalMaterial({ color: accentColor, transparent: true, opacity: 0.5 + Math.random() * 0.3, roughness: 0.4, metalness: 0.05, side: THREE.DoubleSide });
            var mesh = new THREE.Mesh(geo, mat);
            mesh.scale.set(0.55, 1, 1);
            mesh.position.set((Math.random() - 0.5) * 13, (Math.random() - 0.5) * 8, (Math.random() - 0.5) * 10);
            mesh.rotation.set(Math.random() * Math.PI, Math.random() * Math.PI, Math.random() * Math.PI);
            mesh.userData = { spin: new THREE.Vector3((Math.random() - 0.5) * 0.004, (Math.random() - 0.5) * 0.006, (Math.random() - 0.5) * 0.004), phase: Math.random() * Math.PI * 2 };
            scene.add(mesh);
            petals.push(mesh);
          }
          var clock = 0;
          return {
            ...
            animate: function () {
              ...
              petals.forEach(function (p) {
                ...
                p.position.y += Math.sin(clock + p.userData.phase) * 0.002;
              });
            },
          };
        },

        // Bakery — soft, rounded sculptural forms gently breathing (scale
        // pulse), evoking material/texture, never a literal pastry.
        blobs: function (THREE, scene, accentColor, isMobile) {
          var BLOB_COUNT = isMobile ? 4 : 9;
          var blobs = [];
          for (var i = 0; i < BLOB_COUNT; i++) {
            var r = 0.6 + Math.random() * 0.9;
            var geo = new THREE.IcosahedronGeometry(r, 3);
            var mat = new THREE.MeshPhysicalMaterial({ color: accentColor, roughness: 0.55, metalness: 0.05, transparent: true, opacity: 0.85 });
            var mesh = new THREE.Mesh(geo, mat);
            mesh.position.set((Math.random() - 0.5) * 12, (Math.random() - 0.5) * 7, (Math.random() - 0.5) * 9);
            mesh.userData = { breathSpeed: 0.4 + Math.random() * 0.3, phase: Math.random() * Math.PI * 2, spinSpeed: (Math.random() - 0.5) * 0.002 };
            scene.add(mesh);
            blobs.push(mesh);
          }
          var clock = 0;
          return {
            ...
            animate: function () {
              ...
              blobs.forEach(function (b) {
                var s = 1 + Math.sin(clock * b.userData.breathSpeed + b.userData.phase) * 0.06;
                b.scale.set(s, s, s);
                b.rotation.y += b.userData.spinSpeed;
              });
            },
          };
        },

        // Construction/architecture — an architectural wireframe lattice
        // (front + fainter back layer) with a few solid structural "beam"
        // nodes, gentle scroll-linked sway.
        grid: function (THREE, scene, accentColor, isMobile) {
          var cols = isMobile ? 4 : 7, rows = isMobile ? 3 : 5;
          var spacingX = 2.2, spacingY = 1.8;
          var positions = [];
          function addLine(x1, y1, z1, x2, y2, z2) { positions.push(x1, y1, z1, x2, y2, z2); }
          for (var c = 0; c < cols; c++) {
            for (var r = 0; r < rows; r++) {
              var x = (c - (cols - 1) / 2) * spacingX, y = (r - (rows - 1) / 2) * spacingY;
              if (c < cols - 1) addLine(x, y, 0, x + spacingX, y, 0);
              if (r < rows - 1) addLine(x, y, 0, x, y + spacingY, 0);
            }
          }
          var geo = new THREE.BufferGeometry();
          geo.setAttribute('position', new THREE.Float32BufferAttribute(positions, 3));
          var mat = new THREE.LineBasicMaterial({ color: accentColor, transparent: true, opacity: 0.35 });
          var latticeFront = new THREE.LineSegments(geo, mat);
          latticeFront.position.z = 1.5;
          scene.add(latticeFront);
          var latticeBack = new THREE.LineSegments(geo, mat.clone());
          latticeBack.material.opacity = 0.18;
          latticeBack.position.z = -2.5;
          scene.add(latticeBack);
          var beamGeo = new THREE.BoxGeometry(0.12, 0.12, 3.8);
          var beamMat = new THREE.MeshPhysicalMaterial({ color: accentColor, metalness: 0.4, roughness: 0.35 });
          var beamCount = isMobile ? 4 : 10;
          for (var i = 0; i < beamCount; i++) {
            var bc = Math.floor(Math.random() * cols), br = Math.floor(Math.random() * rows);
            var beam = new THREE.Mesh(beamGeo, beamMat);
            beam.position.set((bc - (cols - 1) / 2) * spacingX, (br - (rows - 1) / 2) * spacingY, -0.5);
            scene.add(beam);
          }
          return {
            dollyRange: 6,
            animate: function (scrollProgress) {
              latticeFront.rotation.z = Math.sin(scrollProgress * Math.PI) * 0.03;
              latticeBack.rotation.z = -Math.sin(scrollProgress * Math.PI) * 0.02;
            },
          };
        },

        // Restaurant/dining — soft, warm depth-of-field particles with no
        // hard connecting lines, pure ambiance/depth.
        atmosphere: function (THREE, scene, accentColor, isMobile) {
          var COUNT = isMobile ? 20 : 70;
          var geo = new THREE.SphereGeometry(0.05, 6, 6);
          var mat = new THREE.MeshBasicMaterial({ color: accentColor, transparent: true, opacity: 0.55 });
          var particles = [];
          for (var i = 0; i < COUNT; i++) {
            var mesh = new THREE.Mesh(geo, mat);
            mesh.position.set((Math.random() - 0.5) * 16, (Math.random() - 0.5) * 9, (Math.random() - 0.5) * 14);
            mesh.userData = { v: new THREE.Vector3((Math.random() - 0.5) * 0.006, (Math.random() - 0.5) * 0.004, (Math.random() - 0.5) * 0.006) };
            scene.add(mesh);
            particles.push(mesh);
          }
          return {
            ...
            animate: function () {
              particles.forEach(function (p) {
                ...
                if (Math.abs(p.position.x) > 8) p.userData.v.x *= -1;
                if (Math.abs(p.position.y) > 4.5) p.userData.v.y *= -1;
                if (Math.abs(p.position.z) > 7) p.userData.v.z *= -1;
              });
            },
          };
        },

        // Retail/boutique — a small curated cluster of distinct abstract
        // forms (not one repeated shape) floating and tumbling.
        objects: function (THREE, scene, accentColor, isMobile) {
          var geoFactories = [
            function () { return new THREE.IcosahedronGeometry(0.7, 0); },
            function () { return new THREE.TorusGeometry(0.55, 0.18, 10, 24); },
            function () { return new THREE.OctahedronGeometry(0.7, 0); },
            function () { return new THREE.BoxGeometry(0.9, 0.9, 0.9); },
            function () { return new THREE.ConeGeometry(0.55, 1, 8); },
          ];
          var COUNT = isMobile ? 5 : 11;
          var objs = [];
          for (var i = 0; i < COUNT; i++) {
            var geo = geoFactories[i % geoFactories.length]();
            var mat = new THREE.MeshPhysicalMaterial({ color: accentColor, metalness: 0.3, roughness: 0.4, transparent: true, opacity: 0.88 });
            var mesh = new THREE.Mesh(geo, mat);
            mesh.position.set((Math.random() - 0.5) * 12, (Math.random() - 0.5) * 7, (Math.random() - 0.5) * 9);
            mesh.userData = { spin: new THREE.Vector3((Math.random() - 0.5) * 0.006, (Math.random() - 0.5) * 0.008, (Math.random() - 0.5) * 0.006), bobPhase: Math.random() * Math.PI * 2, baseY: mesh.position.y };
            scene.add(mesh);
            objs.push(mesh);
          }
          var clock = 0;
          return {
            ...
            animate: function () {
              ...
              objs.forEach(function (o) {
                ...
                o.position.y = o.userData.baseY + Math.sin(clock * 0.5 + o.userData.bobPhase) * 0.25;
              });
            },
          };
        },
      ...
      function start3dScene(canvas, recipeKey) {
        try {
          ...
          var buildScene = SCENE_RECIPES[recipeKey] || SCENE_RECIPES.network;
          var sceneHandle = buildScene(THREE, scene, accentColor, isMobile);
          var dollyRange = sceneHandle.dollyRange || 5;
          ...
          function renderFrame() {
            ...
            camera.position.z = baseCameraZ - scrollProgress * dollyRange; // scroll-linked camera dolly
            camera.lookAt(0, 0, 0);

            sceneHandle.animate(scrollProgress);
      ...
      if (particleCanvas && heroForExperimental) {
        var scene3dRecipe = particleCanvas.getAttribute('data-scene-recipe') || 'network';
        if (!isMobile && webglSupported()) {
          loadThreeJs(function (loaded) {
            if (loaded) start3dScene(particleCanvas, scene3dRecipe);
